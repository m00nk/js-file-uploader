...
- **onStartProcessing()** - вызывается из **addFiles()** перед началом
  добавления первого файла. Необходим для избавления от паузы, которая возникает
  с момента вызова **addFiles()** до вызова **onStart()**

- **onEndProcessing()** - вызывается после обработки последнего файла в списке
  переданных в **addFiles()**.

- **onFileAdd(FileInfo fInfo)** - вызывается перед добавлением файла в 
  очередь. 
...
- **onFileAdded(FileInfo fInfo)** - вызывается после успешного добавления файла
  в очередь. Объект **FileInfo** содержит корректно заполненные поля. Если 
  этот метод назначен, то он должен вернуть логическое значение - разрешение на 
  добавление в очередь. Здесь можно добавить дополнительную проверку на 
  размер файла для картинок, котороые подвергаются масштабированию, т.к. в 
  `onFileAdd()` размер итогового файла еще не известен.  
...
- **async addFiles(files)** - Добавляет файлы в очередь. Асинхронный метод. Для
